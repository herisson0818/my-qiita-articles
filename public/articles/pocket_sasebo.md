...
tags: ["GoogleAppsScript", "LINEBot", "Gemini", "個人開発", "OpenData","オープンデータ"]
private: true
updated_at: ''
id: null
...
そんな地元のパパママの悩みを解決したくて、完全個人の趣味で、佐世保市特化型の子育て支援LINEボット**「ポケットさせぼ」**を開発しました！

今回は、**Google Apps Script (GAS)** と **Gemini API (生成AI)** を組み合わせて、**コストゼロ（無料枠）**でどこまで高機能なボットが作れるかに挑戦した記録を共有します。

## 作ったもの：ポケットさせぼ📱
佐世保市の子育て世帯向けに、以下の機能を提供するLINE公式アカウントです。

> **\ 実際に触ってみてください！ /**
> [LINE友だち追加はこちら](https://line.me/R/ti/p/あなたのLINE_ID)
<img src="https://qr-official.line.me/gs/M_892uxzzq_GW.png?oat_content=qr">

* 🏥 **病院検索**: 現在地から、診療科（小児科など）と距離で絞り込み、近い順に表示。
* 🏢 **公共施設検索**: 公園や児童館などを現在地から検索。
...
## 技術スタック🛠️
「スモールスタート」と「運用コストゼロ」にこだわり、サーバーレスで構築しました。

| カテゴリ | 使用技術 |
|:-:|:-|
| **Platform** | LINE Messaging API |
| **Backend** | Google Apps Script (GAS) |
| **Database** | Google スプレッドシート |
| **AI Model** | Google Gemini API (gemini-2.5-flash / gemini-2.5-pro) |
| **Data Source** | BODIK API (自治体オープンデータ) |

## こだわりポイント＆実装の裏側💡

### 1. 病院・施設検索：オープンデータAPIの活用
佐世保市の病院や公共施設のデータは、オープンデータAPI（[BODIK API](https://odcs.bodik.jp/)）を利用しました。
GASからAPIを叩き、ユーザーの**位置情報（緯度経度）**との距離を計算して、近い順にソートして返しています。

実装自体はシンプルでしたが、**「オープンデータならではの壁」**にぶつかりました。

:::note alert 技術だけでは解決できない「データ品質」の課題
APIの仕様がバラバラで正規化が大変だったことに加え、**肝心のデータの中身（質と量）**にも課題がありました。

* **情報の鮮度**: すでに閉院している病院が掲載されたままになっている。
* **情報の粒度**: 「公共施設」カテゴリーに「市営駐車場」なども含まれており、「子供と遊べる場所」を探したいユーザーの意図とずれる（ノイズが混じる）。

これらはデータソースを提供する**行政側の課題**であるため、APIを利用する側の個人開発（クレンジング）では根本的な解決が難しいという「限界」も痛感しました。
:::

### 2. イベント情報：AIによる「自動収集」システム
これが一番の自信作であり、**現在進行系で一番苦労している部分**です。
佐世保のイベント情報はあちこちのサイトに散らばっていて、人間がまとめるのは大変です。そこで、**Gemini Pro**に「イベント収集」を任せることにしました。

**仕組み:**
1.  GASのトリガーで、月2回スクリプトを実行。
2.  Gemini Proに「佐世保の来月のイベントを検索して、JSON形式でまとめて」と指示（プロンプトエンジニアリング）。
3.  AIが集めたデータをGASが解析し、スプレッドシートに自動追記。
...
現在は、AIが出力したデータをGAS側で検証したり、最終的に人間がチェックするフローを挟むなどして対応していますが、\*\*「AIに情報の正確性を100%保証させることの難しさ」\*\*を痛感しています。
...
しかし、ターゲットであるパパママ全員が「AIフレンドリー」なわけではありません。日常的に使い慣れている **LINE** を入り口にすることで、誰でも自然にAIの恩恵を受けられるようにしています。
...
開発は順風満帆ではありませんでした...。
  ...
  * **「既読スルー」問題**
      * LINEボットがうんともすんとも言わなくなる現象。原因は署名検証の失敗や、エラーハンドリングの不足でした。「200 OK」をとりあえず返すことの重要性を学びました。
  * **AIの幻覚（ハルシネーション）**
      * 前述の通り、イベント情報収集でAIが架空のイベントをでっち上げることがありました。プロンプトで「実在確認」を強く指示し、GAS側でもデータの整合性チェックを入れることで精度を上げています。

:::note warning GASの「6分の壁」
高精度なGemini Proモデルは処理が長く、GASの実行時間制限（6分）にぶつかりました。
これに対し、処理を「今月分」と「来月分」に分割してトリガー実行することで解決しました。
:::
...
個人開発の醍醐味は、\*\*「自分の技術で、身近な誰かを助けられること」\*\*だと思います。
「ポケットさせぼ」が、佐世保のパパママの子育てを少しでも楽にできれば嬉しいです！

もし「自分もLINEボット作ってみたい！」という方がいれば、ぜひ **GAS × Gemini** の組み合わせを試してみてください。夢が広がりますよ！
